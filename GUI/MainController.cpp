   ...
   resizeStream(0)
{
    ...
    else
    {
		...
		if (good)
			...
			std::cout << "RealSense: Failed!" << std::endl;

#else
		logReader = new LiveLogReader(logFile, flipColors, LiveLogReader::CameraType::OpenNI2);
		good = ((LiveLogReader *)logReader)->cam->ok();
		if (good)
			std::cout << "OpenNI2: OK!" << std::endl;
		else
			std::cout << "OpenNI2: Failed!" << std::endl;
#endif
    }
	...
	std::cout << "MainController: Get args done!" << std::endl;

	gui = new GUI(logFile.length() == 0, Parse::get().arg(argc, argv, "-sc", empty) > -1);

	std::cout << "MainController: Init GUI done!" << std::endl;
	...
	std::cout << "MainController: Init done!" << std::endl;
}
...
void MainController::launch()
{
    while(good)
    {
        ...
        if(eFusion == 0 || resetButton)
        {
            ...
            logReader->rewind();
			std::cout << "logReader->rewind()" << std::endl;
			...
			std::cout << "ElasticFusion init done." << std::endl;
        }
        else
        {
            break;
